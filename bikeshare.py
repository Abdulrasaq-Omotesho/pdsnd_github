...
def get_filters():
   ...
   Returns:
    ...
    while city not in CITY_DATA.keys():
        print("\nSelect your chosen city:")
        print("\nPlease choose as written from (Chicago, New York City, Washington). \nNote: city name is not case sensitive.")
        # user input converted to lower to standardize them
        
        city = input().lower()

        if city not in CITY_DATA.keys():
            print("\nWrong input champ, please chose from (Chicago, New York City, Washington). \nNote: city name is not case sensitive.")
            time.sleep(1.5)
    ...
    while month not in MONTH.keys():
        print("\nSelect your month of interest, from January to June:")
        print("Enter the month like this: january or JANUARY, february or FEBRUARY, or all/ALL if you have no preference, \nNote: month name is not case sensitive.")
        month = input().lower()

        if month not in MONTH.keys():
            print("\nWrong input champ. try chosing like this from (january or JANUARY to june or JUNE). \nNote: month name is not case sensitive.")
            time.sleep(1.5)
    ...
    while day not in DAY:
        print("\nSelect your day of interest:")
        print("Enter your day of interest like this: monday or MONDAY, tuesday or TUESDAY, or select all if you have no preference.\nNote: day of interest is not case sensitive.")
        day = input().lower()

        if day not in DAY:
            print("\nWrong input champ. Please select from (monday or MONDAY, tuesday or TUESDAY, or select all). \nNote: day of interest is not case sensitive.")
            time.sleep(1.5)
...
def load_data(city, month, day):
    ...
    Args:
        ...
        (str) day -  week to filter by, or "all" to apply no day filter
    Returns:
...
def display_data(df):
    ...
    while view_data not in RESPONSE_LIST:
        ...
        if view_data == "yes":
            ...
            print("it has to be yes or no champ.")
...
def main():
    while True:
        ...
        restart = input('\nHey champ wanna start over again? Enter yes or no.\n')
